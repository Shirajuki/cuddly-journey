...
for i in range(1,25):
    i = str(i).rjust(2, "0")
    payload = f"python3 ./tts_srt_parsing/process_srt.py ./KusuriyanoHitorigoto-{i}.srt False False False && cp ./scripts/output/subbed.srt ep{i}.srt && cp ./scripts/output/filtered.srt ep{i}_filter.srt"
    print(payload)
    os.system(payload)
