...
def extract_srt(video, srt):
    video = FileVideoStream(video)
    ...
    mss = []
    for sub in subs:
        obj = {"from": sub.start.ordinal, "to": sub.end.ordinal}
        mss.append(obj)
    ...
    while frame is not None:
        ...
        if srt_millis - 10 < millis < srt_millis + 10:
            ...
            for l in lines:
                print(l)
            print()
            cv2.imwrite(f"../output/frames/{str(frame_number).rjust(10,'0')}.png", nframe)
...
def main():
    ...
    sys.stdout = FileAndTerminalStream(args.srt_file)
    extract_srt(args.video_file, args.frame_srt_file)
